...
Future<void> main() async {
  ...
  try { await singBox.init(); } catch (e) { debugPrint('sing-box init failed: $e'); }
...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    stateSubscription = singBox.proxyStateStream.listen((state) {
      if (!mounted) return;
      setState(() { running = state == ProxyState.started; message = running ? 'VPN متصل است' : 'VPN قطع است'; });
    });
  }
  ...
  Future<void> _import() async {
    ...
    if (uri == null || !uri.hasScheme || uri.host.isEmpty) { _snack('لینک اشتراک معتبر نیست'); return; }
    setState(() { busy = true; message = 'در حال دریافت و پردازش اشتراک...'; });
    try {
      final profile = await subscriptions.importSubscription(url: raw, name: nameController.text.trim().isEmpty ? null : nameController.text.trim());
  ...
  Future<void> _toggle() async {
    if (selected == null) { _snack('ابتدا اشتراک را اضافه و انتخاب کنید'); return; }
    setState(() => busy = true);
    try {
      if (running) {
  ...
  Future<void> _test() async {
    ...
    setState(() => busy = true);
    try { await singBox.urlTest(groupTag: 'proxy'); _snack('تست تأخیر انجام شد'); }
    catch (_) { _snack('گروه proxy در این پروفایل موجود نیست یا تست ناموفق بود'); }
    finally { if (mounted) setState(() => busy = false); }
  }

  void _select(Profile profile) { if (!running) { profileStorage.setSelectedProfile(profile.id); _reloadProfiles(); } }
  void _delete(Profile profile) { if (!running) { profileStorage.deleteProfile(profile.id); _reloadProfiles(); } }
  void _snack(String text) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(text))); }

  @override
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Light Speed VPN'), centerTitle: true, actions: [IconButton(onPressed: busy ? null : _import, icon: const Icon(Icons.sync))]),
    body: ListView(padding: const EdgeInsets.all(16), children: [
      Card(child: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
        ...
        SizedBox(width: 160, height: 160, child: FilledButton(onPressed: busy ? null : _toggle, style: FilledButton.styleFrom(shape: const CircleBorder()), child: Icon(running ? Icons.stop : Icons.power_settings_new, size: 58))),
        if (selected != null) ...[const SizedBox(height: 14), Text(selected!.name, style: Theme.of(context).textTheme.titleLarge), Text('Profile ID: ${selected!.id}')],
      ]))),
      ...
      ...profiles.map((profile) => Card(child: ListTile(
        ...
        title: Text(profile.name), subtitle: Text('Profile ID: ${profile.id}'),
  ...
  @override
  void dispose() { stateSubscription?.cancel(); urlController.dispose(); nameController.dispose(); super.dispose(); }
}
